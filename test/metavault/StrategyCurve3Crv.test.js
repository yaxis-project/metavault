...
contract('StrategyCurve3Crv', async (accounts) => {
    before(async () => {
        ...
        globalThis.crv = await MockERC20.new('Curve.fi', 'CRV', 18);
        globalThis.gauge = await MockCurveGauge.new(globalThis.t3crv.address);
        globalThis.minter = await MockCurveMinter.new(globalThis.crv.address);
        globalThis.strategy = await StrategyCurve3Crv.new(
