...
describe('StrategyGenericVault', () => {
    ...
    before(async () => {
        ...
        await deployments.deploy('MockGenericVault', {
            from: deployer,
            contract: 'MockGenericVault',
            args: [usdt.address]
        });
        const Vault = await deployments.get('MockGenericVault');
        await deployments.deploy('StrategyGenericVault', {
            from: deployer,
            args: [
                'Generic Vault: USDT',
                Vault.address,
        ...
        const Strategy = await deployments.get('StrategyGenericVault');
    ...
    it('should deploy with initial state set', async () => {
        expect(await strategy.name()).to.equal('Generic Vault: USDT');
    ...
    it('should withdraw to DAI', async () => {
        ...
        expect(await dai.balanceOf(user)).to.be.least(ether('993'));
    });
