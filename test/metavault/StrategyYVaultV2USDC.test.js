...
describe('StrategyYVaultV2USDC', () => {
    ...
    it('should deploy with initial state set', async () => {
        expect(await strategy.name()).to.equal('YVaultV2: USDC');
        expect(await strategy.want()).to.equal(usdc.address);
    ...
    it('should deposit USDT', async () => {
        ...
        expect(await strategy.balanceOfPool()).to.be.above('1000000');
        expect(await controller.balanceOf(t3crv.address)).to.be.above(ether('9'));
